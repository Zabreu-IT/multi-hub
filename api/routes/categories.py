...
@router.post("",response_model=CategoryOut,dependencies=[Depends(require_admin(["owner", "admin"]))])
def create(data:CategoryIn,db:Session=Depends(get_db)):
    ...
    return x
@router.put("/{id}",response_model=CategoryOut,dependencies=[Depends(require_admin(["owner", "admin"]))])
