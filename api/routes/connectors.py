...
@router.post("",response_model=ConnectorOut,dependencies=[Depends(require_admin())])
...
@router.post("/{id}/sync",dependencies=[Depends(require_admin())])
...
@router.get("/{id}/status",dependencies=[Depends(require_admin())])
